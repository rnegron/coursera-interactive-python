...
def draw(canvas):
    global time, started, lives, score, rock_speed
...
def rock_spawner():
    ...
    if len(rock_group) < 12 and started:
        ...
        rock_vel = [random.random() * .6 - .3 + (rock_speed * check), random.random() * .6 - .3 + (rock_speed * check)]
...
difficulty_timer = simplegui.create_timer(5000.0, add_difficulty)
